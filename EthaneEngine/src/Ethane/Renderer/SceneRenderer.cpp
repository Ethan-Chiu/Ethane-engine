...
namespace Ethane {
	...
	void SceneRenderer::Init()
	{
		...
		// TODO: test
		m_CompositeMaterial = Material::Create(ShaderLibrary::Get("SceneComposite"), "Compodite material");
	...
	void SceneRenderer::BeginScene(const EditorCamera& camera)
	{
		...
		// 
		// 	uint32_t quarterWidth = (uint32_t)glm::ceil((float)m_ViewportWidth / 4.0f);
		// 	uint32_t quarterHeight = (uint32_t)glm::ceil((float)m_ViewportHeight / 4.0f);
		...
		// });
		VulkanRendererAPI::SetUniform(0, 0, &cameraData.ViewProjection, sizeof(cameraData.ViewProjection), 0);
